...
class SearchFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        searchViewModel.searchLiveData.observe(viewLifecycleOwner) { renderDataFromRemoteServer(it) }
        val moviesSearchList = binding.searchContainer
        ...
        binding.actionAdult.isChecked = searchSettings.getBoolean(IS_ADULT_KEY, false)
        binding.actionAdult.setOnCheckedChangeListener { _, isChecked ->
            readSearchSettings(searchSettings, isChecked)
        }
        binding.searchField.setOnQueryTextListener(object : OnQueryTextListener {
    ...
    private fun renderDataFromRemoteServer(appState: MovieAppState) {
        when (appState) {
            is MovieAppState.Error -> { val error = appState.error }
